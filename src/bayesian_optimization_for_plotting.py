...
class bayesian_optimization:
    def __init__(self, objective, domain, arg_max = None, n_workers = 1,
        ...
        # print(" i am here initing")
        # Domain
        self.domain = domain    #shape = [n_params, 2]
        self._dim = domain.shape[0]
        self._grid_density = grid_density
        grid_elemets = []
        # print(" i am here initing 1.1")
        if self._dim >= 5:
            # print("og grid density", self._grid_density)
            # print("change grid density!")
...
class TorchGPModel():
    def __init__(self, X, Y, alg_name=None, n_workers = None, objective_name = None):
        ...
        # if objective_name == 'Boston' or objective_name == 'bCancer':
        ...
        # if objective_name == 'Boston':
        #     self.model.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.MaternKernel(ard_num_dims = 4))
        #     self.model.covar_module.base_kernel.lengthscale = torch.tensor([1.0, 0.05, 0.1,0.1])
        ...
        if objective_name == 'robot3d':
            ...
            # print("self module length scale if Boston", self.model.covar_module.base_kernel.lengthscale.item())
            # self.train()
        # if objective_name == 'robot3d':
        #     self.model.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.MaternKernel(ard_num_dims = 3))
        #     self.model.covar_module.base_kernel.lengthscale = torch.tensor([1.0, 1.0, 1.0])
        # if objective_name == 'robot4d':
        #     self.model.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.MaternKernel(ard_num_dims = 4))
        #     self.model.covar_module.base_kernel.lengthscale = torch.tensor([1.0, 1.0, 0.1, 0.25])
        #     print("initial output scale for robot3d", self.model.covar_module.outputscale)
        #     self.model.covar_module.outputscale = 0.2
        #     print("initial output scale for robot3d now", self.model.covar_module.outputscale)
        #     # print("self module length scale if Boston", self.model.covar_module.base_kernel.lengthscale.item())
    ...
    #      ackley_3d   # noises.append(self.model.likelihood.noise.item())
...
class BayesianOptimizationCentralized(bayesian_optimization):
    ...
    def _TS_UCB_seq(self, a, x, n, radius=0.1,n_random = 1000,n_restarts = -1):
        ...
        for i in range(self.n_workers- 1):
            ...
            # print("num of min ts_ucb, iter %d, outer iter %d" % (i,n), (fstar_hat[i+1] - mu)[np.argmin(ts_ucb)])
            # print("mu of min ts_ucb", mu[np.argmin(ts_ucb)])
            # print("mu of a neighbor of min ts_ucb", self.model.predict(x[np.argmin(ts_ucb)] + np.asarray([1e-4,1e-4,1e-4])))
            # print("x of min ts_ucb, iter %d, outer iter %d" % (i,n), x[np.argmin(ts_ucb)])
            # print("min ts Ucb dtype", (x[np.argmin(ts_ucb)]).dtype)
            # print("true objextive at argmin ts_ucb", self.objective(x[np.argmin(ts_ucb)]))
...
##  f_star_hat does not get resampled per new agent
    ...
    def optimize(self, n_iters, n_runs = 1, x0=None, n_pre_samples=15, random_search=100, plot = False, noise_scale = 1e-3):
        ...
        #randomize after initial iterates chosen.
