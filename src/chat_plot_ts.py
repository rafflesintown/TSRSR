...
outdir = os.path.join("visualization", "ts_single")
...
# ----------------------------------------------------
X_final = np.vstack([X_train, x1])
y_final = np.append(y_train, [real_y1])
...
fig1, (ax3, ax1_bot) = plt.subplots(2, 1, figsize=(8, 4.5))
...
ax1_bot.plot(X_grid, sample1, 'c-', label=r'TS Sample $\tilde{f}_{t}$')
...
# # ax2_top.plot(X_grid, mean_init, 'b-', label=r'$\mu_0(x)$')
