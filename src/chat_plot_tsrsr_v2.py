...
custom_fill1 = mpatches.Patch(color = 'blue', alpha = 0.2, label = r'$\mu_t(x)\pm 1.5\sigma_t(x)$')                    
...
for h, lab in zip(handles, labels):
    if lab == r'$\mu_t(x)\pm 1.5\sigma_t(x)$':
        new_handles.append(custom_fill1)
    else:
...
# print("I got through this part")
# # plt.axis('off')  # Optionally turn off the axis for a clean legend.
# # plt.tight_layout()
figLegend.savefig(os.path.join(output_dir, 'legend_ax1_top.pdf'))
...
figLegend.savefig(os.path.join(output_dir, 'legend_ax1_bot.pdf'))
plt.close(fig1)
...
custom_fill2 = mpatches.Patch(color = 'green',alpha = 0.2, label=r'$\mu_t(x)\pm 1.5\sigma_t(x\,|\,x_{t,1})$')
...
new_labels = []
for h, lab in zip(handles, labels):
    if lab == r'$\mu_t(x)\pm 1.5\sigma_t(x\,|\,x_{t,1})$':
        new_handles.append(custom_fill2)
    else:
        new_handles.append(h)
    new_labels.append(lab)
...
# # plt.axis('off')  # Optionally turn off the axis for a clean legend.
# # plt.tight_layout()
figLegend.savefig(os.path.join(output_dir, 'legend_ax2_top.pdf'))
...
figLegend.savefig(os.path.join(output_dir, 'legend_ax2_bot.pdf'))
plt.close(fig2)
...
custom_fill3 = mpatches.Patch(color = 'maroon',alpha = 0.2, label=r'$\mu_{t+1}(x)\pm 1.5\sigma_{t+1}(x)$')
...
new_labels = []
for h, lab in zip(handles, labels):
    if lab == r'$\mu_{t+1}(x)\pm 1.5\sigma_{t+1}(x)$':
        new_handles.append(custom_fill3)
    else:
        new_handles.append(h)
    new_labels.append(lab)
...
figLegend.legend(new_handles, new_labels, loc='center', fontsize=12)
# plt.axis('off')  # Optionally turn off the axis for a clean legend.
figLegend.savefig(os.path.join(output_dir,'legend_ax3.pdf'))
