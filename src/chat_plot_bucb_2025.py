...
output_dir = os.path.join("visualization_merl", "bucb")
...
# ----------------------------------
# Figure 0: Save plot for Step 0 (before any point picked)
# ----------------------------------
fig1, ax  = plt.subplots(1, 1, figsize=(8, 4), sharex=True, sharey=True)

# Top: GP Posterior at Step 1.
ax.plot(X_grid, y_true, r'r--', label=r'\textbf{True function}')
ax.scatter(X_train, y_train, c='black', zorder=10, label=r'\textbf{Prior obs}')
ax.plot(X_grid, mean1, r'b-', label=r'$\mu_t(x)$')
ax.fill_between(X_grid.ravel(), mean1 - beta*std1, mean1 + beta*std1,
...
ax.set_title(r'\textbf{GP at round $t$}')
ax.set_ylabel(r'$f(x)$')
ax.set_xlabel(r'$x$')
# plt.subplots_adjust(right=0.8)
# ax.legend(fontsize=10, loc = 'upper right',bbox_to_anchor=(1.5, 1.2))
...
plt.tight_layout()
fig1.savefig(os.path.join(output_dir, "BUCB_Step0.pdf"))
plt.close(fig1)
...
ax1_top.fill_between(X_grid.ravel(), mean1 - beta*std1, mean1 + beta*std1,
                     color='blue', alpha=0.2, label=r'$\mu_t(x)\pm 1.5\sigma_t(x)$')
# ax1_top.axvline(x1, color='blue', linestyle='--', label=r'$x_{t,1}$')
ax1_top.axvline(x1, color='blue', linestyle='--')
ax1_top.scatter(x1, y1_fantasy, color='blue', s=100, zorder=10)
# ax1_top.axhline(f_star_1, color='gray', linestyle='--', label=r'$f^*_{\mathrm{TS}}$')
...
ax1_bot.plot(X_grid, ucb1, r'k-', label=r'\textbf{Acquisition: }$\mu_t(x)+1.5\sigma_t(x)$')
# ax1_bot.axvline(x1, color='blue', linestyle='--', label=r'$x_{t,1}$')
ax1_bot.axvline(x1, color='blue', linestyle='--')
ax1_bot.set_title(r'\textbf{BUCB Acquisition}')
ax1_bot.set_xlabel(r'$x$')
ax1_bot.set_ylabel(r'Acq')
# ax1_bot.legend(fontsize=8,loc = 'upper right')
