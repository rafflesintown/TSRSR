...
def f_true(x):
	return(np.sin(3* x + 0.8) + np.cos(x - 0.1) + np.cos(5*(x-0.3)) + 2 * np.cos(x-0.5))
...
def plot_acq():
	...
	plt.savefig("posterior_before_acq.pdf", bbox_inches='tight')

	x_next = x[np.argmax(x_mean + x_std)]
	plt.scatter(x_next, np.max(x_mean + x_std), 
		s = 50,marker = "^", color = "magenta")
	plt.savefig("posterior_with_acq.pdf", bbox_inches='tight')
	plt.close()
	...
	plt.ylabel(r"$f(x)$")
	plt.legend(loc = 'lower right')
	plt.savefig("posterior_after_acq.pdf", bbox_inches='tight')
...
def plot_batch():
	...
	plt.savefig("prior_with_ES_batch.pdf", bbox_inches='tight')
	plt.close()
	...
	# plt.ylabel(r"$f(x)$")
	# plt.legend(loc = 'lower center')
	plt.savefig("posterior_with_ES_batch.pdf", bbox_inches='tight')
	plt.close()
	...
	# 	axs.plot(domain, f_true(domain), '--', 
	# 		label = r"true $f$", color = "grey")
	# 	axs.plot(domain, x_mean, 
	# 		label = r'mean($\mu (\cdot))$', color = 'blue')
	# 	axs.scatter([pt1, pt2],[sin_1d(pt1), sin_1d(pt2)], 
	# 	color = "black", s = 50, marker = "X")
	# 	axs.scatter([new_pt1, new_pt2], [sin_1d(new_pt1), sin_1d(new_pt2)], 
	# 		s = 50,marker = "X", color = "magenta")
	# 	axs.fill_between(domain, x_mean + x_std, x_mean - x_std, 
	# 	label = r"$\mu(\cdot) \pm \sigma(\cdot)$", alpha = 0.2, color = 'blue')
	# 	axs.axis("off")



	# X = np.vstack([X, np.asarray([[new_pt1],[new_pt2]])])
	# Y = np.append(Y, (sin_1d(new_pt1), sin_1d(new_pt2)))
	# model.fit(X,Y)
	# x_mean, x_var = model.predict(x, return_std=True)
	# x_std = np.sqrt(x_var)
	# plt.plot(domain, np.sin(3. * domain), '--', label = r"true $f$", color = "grey")
	...
	# plt.scatter([pt1, pt2],[sin_1d(pt1), sin_1d(pt2)], 
	# 	color = "black", s = 50, marker = "X")
	# plt.scatter([new_pt1, new_pt2], [sin_1d(new_pt1), sin_1d(new_pt2)], 
	# 	s = 50,marker = "X", color = "magenta")
	# plt.fill_between(domain, x_mean + x_std, x_mean - x_std, 
	# 	label = r"$\mu(\cdot) \pm \sigma(\cdot)$", alpha = 0.2, color = 'blue')
	# plt.xlabel(r"$x$")
	# plt.ylabel(r"$f(x)$")
	# plt.legend(loc = 'lower center')
	# plt.savefig("posterior_after_batch.pdf", bbox_inches='tight')
	# plt.close()
